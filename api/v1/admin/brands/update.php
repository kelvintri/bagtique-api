<?php
error_log('=== Admin Update Brand API Request ===');
...
try {
    ...
    try {
        ...
        $stmt = $conn->prepare("SELECT id FROM brands WHERE slug = ? AND id != ?");
        $stmt->execute([$slug, $brand_id]);
        if ($stmt->fetch()) {
            $slug .= '-' . time();
        }
    ...
    error_log('Database error in admin/brands/update.php: ' . $e->getMessage());
    ...
    error_log('Error in admin/brands/update.php: ' . $e->getMessage());
